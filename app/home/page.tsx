import Link from 'next/link'
import { Button } from '@/components/ui/button'

import { InfiniteSlider } from '@/components/ui/infinite-slider'
import { ProgressiveBlur } from '@/components/ui/progressive-blur'
import { ChevronRight } from 'lucide-react'
    

export default function Home() {
  return (
    <>
      
            <main className="overflow-x-hidden">
                <section>
                    <div className="py-24 md:pb-32 lg:pb-36 lg:pt-72">
                        <div className="relative mx-auto flex max-w-7xl flex-col dark:bg-amber px-6 lg:block lg:px-12">
                            <div className="mx-auto max-w-lg text-center lg:ml-0 lg:max-w-full lg:text-left">
                                <h1 className="mt-8 max-w-2xl text-bold text-5xl md:text-6xl lg:mt-16 text-white xl:text-7xl">Welcome to Affordable Rides</h1>
                                <p className="mt-8 max-w-2xl text-balance  text-lg text-white/90">
At Affordable Rides, we believe that everyone deserves a reliable vehicle without breaking the bank.
</p>

                                <div className="mt-12 flex flex-col items-center justify-center gap-2 sm:flex-row lg:justify-start">
                                    <Button
                                        asChild
                                        size="lg"
                                        className="h-12 rounded-full pl-5 pr-3 text-base">
                                        <Link href="/Knowmore">
                                            <span className="text-nowrap ">Know more</span>
                                            <ChevronRight className="ml-1" />
                                        </Link>
                                    </Button>
                                    <Button
                                        key={2}
                                        asChild
                                        size="lg"
                                        variant="ghost"
                                        className="h-12 rounded-full px-5 text-base hover:bg-zinc-950/5 dark:hover:bg-white/5">
                                        <Link href="https://wa.link/ou0ny7">
                                            <span className="text-nowrap text-amber-100">Request an inspection</span>
                                        </Link>
                                    </Button>
                                </div>
                            </div>
                        </div>
                        <div className="aspect-2/3 absolute inset-1 -z-10 overflow-hidden rounded-3xl border border-black/10 lg:aspect-video lg:rounded-[3rem] dark:border-white/5">
                            <img src="/images/pan.jpg" alt="a car" />
                        </div>
                    </div>
                </section>
                <section className="bg-background pb-2">
                    <div className="group relative m-auto max-w-7xl px-6">
                        <div className="flex flex-col items-center md:flex-row">
                            

                                <div className="bg-linear-to-r from-background absolute inset-y-0 left-0 w-20"></div>
                                <div className="bg-linear-to-l from-background absolute inset-y-0 right-0 w-20"></div>
                                <ProgressiveBlur
                                    className="pointer-events-none absolute left-0 top-0 h-full w-20"
                                    direction="left"
                                    blurIntensity={1}
                                />
                                <ProgressiveBlur
                                    className="pointer-events-none absolute right-0 top-0 h-full w-20"
                                    direction="right"
                                    blurIntensity={1}
                                />
                                <div className="md:max-w-44 md:border-r md:pr-6">
                                <p className="text-end text-sm">Powering the best teams</p>
                            </div>
                            <div className="relative py-6 md:w-[calc(100%-11rem)]">
                                <InfiniteSlider
                                    speedOnHover={20}
                                    speed={50}
                                    gap={112}>
                                    <div className="flex ">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/ford-logo.png"
                                            alt="Ford Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>

                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/bmw-logo.png"
                                            alt="Bmw Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/honda-logo.png"
                                            alt="Honda Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/jaguar-logo.png"
                                            alt="Jaguar Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/nissan-logo.png"
                                            alt="Nissan Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/maserati-logo.png"
                                            alt="Maserati Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/subaru-logo.png"
                                            alt="Subaru Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>

                                    <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/tesla-logo.png"
                                            alt="Tesla Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                     <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/jeep-logo.png"
                                            alt="Jeep Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                     <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/bentley-logo.png"
                                            alt="Bentley Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                     <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/hyundai-logo.png"
                                            alt="Hyundai Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                     <div className="flex">
                                        <img
                                            className="mx-auto h-20 w-fit dark:invert"
                                            src="/images/audi-logo.png"
                                            alt="Audi Logo"
                                            height="auto"
                                            width="auto"
                                        />
                                    </div>
                                </InfiniteSlider>
                            </div>
                        </div>
                    </div>
                </section>
            </main>
     
    </>
  );
}
